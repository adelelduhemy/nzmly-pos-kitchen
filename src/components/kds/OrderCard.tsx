...
interface OrderCardProps {
    order: Order;
    onStatusUpdate: (orderId: string, newStatus: string, version: number) => void;
    isUpdating?: boolean;
}

const OrderCard = React.forwardRef<HTMLDivElement, OrderCardProps>(
    ({ order, onStatusUpdate, isUpdating }, ref) => {
                ...
                <Card className={cn('border-2', getStatusColor())}>
                    ...
                    <CardContent className="space-y-3">
                        ...
                        {order.status !== 'served' && (
                            <Button
                                className="w-full"
                                onClick={() => onStatusUpdate(order.id, getNextStatus(), order.version)}
                                disabled={isUpdating}
                            >
